console.log("Beyond20: Roll21 module loaded......");
  ...
  function rollAttack(request, custom_roll_dice = "") {
    ...
    if (request.damages && request.damages.length > 0 &&
      ...
      if (request.includeTotal){
        let numDamages = request.damages.length;
        let i = 1;
        properties["desc"] = "Total: [[";
        properties["desc"] += properties["dmg1"];
        properties["dmg1"] = "$[[0]]"
        properties["dmg2"] = properties["dmg2"]?.replace(/\[\[.*?]]/g, str => {
          properties["desc"] += " + " + str;
          return `$[[${i++}]]`;
        });
        properties["crit1"] = properties["crit1"].replace(/\[\[.*?]]/g, str => {
          properties["desc"] += " + ((" + str;
          return `$[[${i++}]]`;
        });
        properties["crit2"] = properties["crit2"]?.replace(/\[\[.*?]]/g, str => {
          properties["desc"] += " + " + str;
          return `$[[${i++}]]`;
        });
        properties["desc"] += `) * [[{${properties["r1"]},0}>${19 + parseInt(request["to-hit"].match(/\d+/)[0])}]])]]`;
        properties["r1"] = `$[[${i++}]]`;
      }
      if (properties["desc"] === ""){
